...
class MongoDBInterface(DatabaseInterface):
    ...
    def browseNearestCars(self, location: tuple[str, str], distance) -> list["Car"]:
        cars = []
        for car in self.rentalDb["Car"].find({"status": {"$not": "DELETED"}}):
    ...
    def browseNearestLocations(
        ...
        locations = []
        for location_ in self.rentalDb["Location"].find({"status": {"$not": "DELETED"}}):
            if calculate_gps_distance((float(location[0]), float(location[1])),
    ...
    def startRental(self, userId, carId):
        ...
        rentalId = ObjectId()
        if self.rentalDb["User"].find_one_and_update({"$set": {"currentRental": {
                '_id': rentalId,
    ...
    def serviceCar(self, carId, userId, locationId, description=""):
