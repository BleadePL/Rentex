...
def populateDataBase(db:MongoClient, howManyCars, howManyUsers, howManyCreditCards, howManyLocations, howManyRentals):
    ...
    resCars = db["Car"].insert_many(cars[0])
    print(len(resCars.inserted_ids))
    resUsers = db["User"].insert_many(users[0])
    print(len(resUsers.inserted_ids))
    resLocations = db["Location"].insert_many(locations[0])
